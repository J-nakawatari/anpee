 ...
 * 開発・検証用のエンドポイント
 */
router.get('/user-debug-info', authenticate, async (req: AuthRequest, res: Response) => {
  try {
 ...
 * 開発・検証用のエンドポイント
 */
router.post('/sync-stripe-subscription', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.userId
    ...
    try {
      await (getStripeService() as any).handleSubscriptionUpdate(subscription)
