...
export const updateUserProfile = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { phone }, { new: true, runValidators: true }).select('-password -emailVerificationToken -passwordResetToken');
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'ユーザーが見つかりません'
...
export const requestEmailChange = async (req, res) => {
    try {
        ...
        if (user) {
            await emailService.sendEmailChangeVerification(newEmail, user.name, `${process.env.FRONTEND_URL}/verify-email?token=${token}`);
        }
        res.json({
            success: true,
