...
router.get('/history', authenticate, async (req, res) => {
  try {
    ...
    // フィルター条件
    if (elderlyId && mongoose.Types.ObjectId.isValid(elderlyId as string)) {
  ...
  try {
    ...
    stats.forEach((stat: any) => {
      ...
      if (type in formattedStats) {
        const typeKey = type as keyof typeof formattedStats;
        formattedStats[typeKey][status as keyof typeof formattedStats[typeof typeKey]] = stat.count;
