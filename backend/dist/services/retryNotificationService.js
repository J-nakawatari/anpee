...
class RetryNotificationService {
    ...
    async checkAndSendRetryNotifications() {
        try {
            ...
            for (const [userId, userHistory] of userHistories) {
                ...
                if (!user || !user.notificationSettings?.retrySettings?.maxRetries)
                    continue;
    ...
    // 管理者通知が必要かどうか判定
    shouldNotifyAdmin(latestResponse, latestHistory, retrySettings) {
        // 今日の応答がある場合は通知不要
        if (latestResponse && latestResponse.status === 'success') {
            return false;
        }
